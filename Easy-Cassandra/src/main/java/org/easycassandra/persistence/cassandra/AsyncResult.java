...
enum AsyncResult {
    ...
    public <T> void runSelect(ResultAsyncCallBack<List<T>> callBack,
            ResultSetFuture result, int offset, Class<?> class1) {

        result.addListener(new RunSelectRunnable<>(callBack, result, offset, class1),
                service);
    }
    ...
    class RunSelectRunnable<T> implements Runnable {
        ...
        public RunSelectRunnable(ResultAsyncCallBack<List<T>> callBack,
                ResultSetFuture resultSetFuture, int offset, Class<?> beanClass) {
        ...
        public void run() {
            ...
            try {
                ...
                List<T> beans = (List<T>) RecoveryObject.INTANCE.recoverObjet(beanClass, result, this.offset);
    ...
    class RunFindKeyRunnable<T> implements Runnable {
        ...
        public void run() {
            ...
            try {
                result = resultSetFuture.get();
                @SuppressWarnings("unchecked")
                List<T> beans = (List<T>) RecoveryObject.INTANCE.recoverObjet(beanClass, result, 0);
