...
public class PersistenceSimpleImpl extends PersistenceBuilderImpl implements Persistence  {
    ...
    public <T, I> List<T> findByIndex(String indexName, I index, Class<T> bean) {
        return findByIndex(indexName, index, bean, 0);
    }

    @Override
    public <T, I> List<T> findByIndex(String indexName, I index, Class<T> bean, int offset) {
        return command.findByIndex(indexName, index, bean, session, offset);
    }
    
    @Override
    public <T, I> List<T> findByIndex(I index, Class<T> bean) {
        return findByIndex(index, bean, 0);
    }

    @Override
    public <T, I> List<T> findByIndex(I index, Class<T> bean, int offset) {
        return command.findByIndex(index, bean, session, offset);
    }
    ...
    public <T, I> List<T> findByIndex(String indexName, I index, Class<T> bean,
            ConsistencyLevel consistency) {
        return findByIndex(indexName, index, bean, consistency, 0);
    }

    @Override
    public <T, I> List<T> findByIndex(String indexName, I index, Class<T> bean,
            ConsistencyLevel consistency, int offset) {
        return command.findByIndex(indexName, index, bean, 
					session, consistency, offset);
    }
    
    @Override
    public <T, I> List<T> findByIndex(I index, Class<T> bean, ConsistencyLevel consistency) {
        return findByIndex(index, bean, consistency, 0);
    }

    @Override
    public <T, I> List<T> findByIndex(I index, Class<T> bean, ConsistencyLevel consistency, int offset) {
        return command.findByIndex(index, bean, session, consistency, offset);
    }
    
    @Override
    public <T, I> List<T> findByKeyAndIndex(Object key, I index, Class<T> bean) {
        return findByKeyAndIndex(key, index, bean, 0);
    }

    @Override
    public <T, I> List<T> findByKeyAndIndex(Object key, I index, Class<T> bean, int offset) {
        return command.findByKeyAndIndex(key, index, bean, session, offset);
    }
    
    @Override
    public <T, I> List<T> findByKeyAndIndexRange(Object key, Object indexStart,
            Object indexEnd, boolean inclusive, Class<T> bean) {
        return findByKeyAndIndexRange(key, indexStart, indexEnd,
                inclusive, bean, 0);
    }

    @Override
    public <T, I> List<T> findByKeyAndIndexRange(Object key, Object indexStart,
            Object indexEnd, boolean inclusive, Class<T> bean, int offset) {
        return command.findByKeyAndIndexRange(key, indexStart, indexEnd,
                inclusive, bean, session, offset);
    }
