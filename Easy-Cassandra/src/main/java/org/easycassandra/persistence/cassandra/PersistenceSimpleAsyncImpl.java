...
public class PersistenceSimpleAsyncImpl extends PersistenceBuilderImpl implements PersistenceAsync {
    ...
    public <T, I> void findByIndex(String indexName, I index, Class<T> bean, 
            ResultAsyncCallBack<List<T>> resultCallBack) {
        findByIndex(indexName, index, bean, 0, resultCallBack);
    }
    
    @Override
    public <T, I> void findByIndex(String indexName, I index, Class<T> bean, int offset,
            ResultAsyncCallBack<List<T>> resultCallBack) {
        command.findByIndex(indexName, index, bean, session, offset, resultCallBack);
    }
    ...
    public <T, I> void findByIndex(String indexName, I index, Class<T> bean,
                       ConsistencyLevel consistency, 
                       ResultAsyncCallBack<List<T>> resultCallBack) {
        findByIndex(indexName, index, bean, consistency, 0, resultCallBack);
    }
    ...
    public <T, I> void findByIndex(I index, Class<T> bean, 
            ResultAsyncCallBack<List<T>> resultCallBack) {
        findByIndex(index, bean, 0, resultCallBack);
    }
    
    @Override
    public <T, I> void findByIndex(I index, Class<T> bean, int offset,
            ResultAsyncCallBack<List<T>> resultCallBack) {
        command.findByIndex(index, bean, session, offset, resultCallBack);
    }
