...
class FindByIndexQuery extends FindByKeyQuery {
    ...
    public <T, I> List<T> findByIndex(I index, Class<T> bean, Session session,
        ...
        return findByIndex(fields.get(0).getName(), index, bean, session, consistency, offset);
    }
	...
	public <T, I> List<T> findByIndex(String indexName, I key, Class<T> bean,
        ...
        return findByIndex(indexName, key, bean, session, consistency, 0);
    }
	
    public <T, I> List<T> findByIndex(String indexName, I key, Class<T> bean,
            Session session, ConsistencyLevel consistency, int offset) {
        QueryBean byKeyBean = createQueryBean(bean, consistency);
        return executeConditions(indexName, key, bean, session, byKeyBean, offset);
    }

    private <T> List<T> executeConditions(String indexName, Object key,
            Class<T> bean, Session session, QueryBean byKeyBean, int offset) {
