...
@EntityType(withRootClient = true, withRootOrg = true, withRootApp = true, 
		alwayRootOrg = true)
@Caption("unit")
public class MUnit extends BaseEO<MUnitKey, UUID> {
	/**
	 * 
	 */
	...
	@EntityLinked(entityClazz = MUnit.class, keyClazz = MUnitKey.class, pkClazz = UUID.class, 
			idProperty = MUnitKey.KEY_NAME, captionProperty = "name", emptyValue = DEFAULT_PARENT_STRING_VALUE, 
	...
	public MUnitKey getKey() {
		return key;
	}

	public void setKey(MUnitKey key) {
		this.key = key;
	}
