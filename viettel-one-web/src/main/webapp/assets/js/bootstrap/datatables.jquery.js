...
(/** @lends <global> */function( $ ) {
	...
	{
		var oCol = oSettings.aoColumns[iCol];
		if ("rownum" == oCol.mData) {
			if ('display' == sSpecific)
				return '<span class=\'rownum\'>' + (iRow + 1) + '</span>';
			else if ('type' == sSpecific)
				return 'numeric';
			else
				return (iRow + 1);
		}
	...
	function _fnCreateTr ( oSettings, iRow, nTrIn, anTds )
	{
		...
		if ( row.nTr === null )
		{
			...
			if (oSettings.sIdColumn != null && oSettings.sIdColumn != '') {
				var mData = _fnGetObjectDataFn(oSettings.sIdColumn);
				var idValue = mData(rowData, 'display');
				nTr.setAttribute("idvalue", idValue);
			}
			
			// PhatPQ
			/*var rownumTd = document.createElement('td');
			nTr.appendChild( rownumTd );
			rownumTd.innerHTML = (iRow + 1);
			...
			for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
			{
				...
				{
					//alert(_fnGetCellData( oSettings, iRow, i, 'display' ));
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		for ( var i=0 ; i<aDom.length ; i++ )
		{
			...
			else if ( cOption == 'p' && features.bPaginate )
			{
				...
				if (oSettings.sToolbarId != '') {
					featureNode = $('<div/>').append($(oSettings.sToolbarId));
				}
			}
			...
			.append(
				$(_div, { 'class': classes.sScrollHead } )
					.css( {
						...
						position: 'relative',
						border: 0,
						width: '100%' //scrollX ? size(scrollX) : '100%'
					} )
					.append(
						$(_div, { 'class': classes.sScrollHeadInner } )
							.css( {
								'box-sizing': 'content-box',
								width: '100%' //scroll.sXInner || '100%'
							} )
							.append(
								headerClone
									.removeAttr('id')
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			var sToolbar = this.getAttribute('toolbar');
			var sIdColumn = this.getAttribute('idcolumn');
