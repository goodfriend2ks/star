...
ConsumerTokenServices, InitializingBean {
	...
	private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
		...
		{
			VOUser user = (VOUser)principal;
			LinkedHashMap<String, Object> userInfo = new LinkedHashMap<>();
			userInfo.put("UserName", user.getUsername());
			userInfo.put("FullName", user.getFullName());
			userInfo.put("Email", user.getEmail());
			userInfo.put("AD_Client_ID", user.getAd_Client_ID());
			userInfo.put("AD_Org_ID", user.getAd_Org_ID());
			userInfo.put("AD_User_ID", user.getAd_User_ID());
